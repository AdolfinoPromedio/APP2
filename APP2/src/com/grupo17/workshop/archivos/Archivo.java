...
public class Archivo { //Clase archivo, la cual tiene de atributo el directorio del archivo y de función leer el csv dado, el cual es almacenado en un ArrayList.
    ...
    public List<Libros> leerDatos(){ //Función que devuelve un ArrayList del objeto Libros. Se lee del csv dado.

        String line = ""; //Este string irá guardando las lineas del csv.
        List<Libros> libro = new ArrayList<Libros>(); //Creamos una lista vacía.
        ...
        try {
            ...
            while ((line = obj.readLine()) != null){ //Se le dice que linea tome el valor de cada linea del archivo csv. Este ciclo finaliza cuando no hay más que leer.
                ...
                if (values.length==1){ //Si no hay comillas en la línea, entra en esta condición.
                    ...
                    Libros texto = new Libros(aux[0], aux[1], Integer.parseInt(aux[2]), Integer.parseInt(aux[3]), aux[4], Integer.parseInt(aux[5]), aux[6], aux[7]); //Luego cada valor de la lista es dado para que se cree un nuevo libro
                    ...
                    Libros texto = new Libros(values[1], aux2[1], Integer.parseInt(aux2[2]), Integer.parseInt(aux2[3]), aux2[4], Integer.parseInt(aux2[5]), aux2[6], aux2[7]); //Se le dan los nuevos valores para que se cree un nuevo libro.
    ...
    public void rewrite(@NotNull List<Libros> libro){ //Procedimiento que reescribe el csv con las modificaciones al Arraylist de libros.
        try {
